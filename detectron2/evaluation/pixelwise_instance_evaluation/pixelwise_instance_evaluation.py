...
def assignGt2Preds(gtInstancesOrig, gtImage, predInfo, args):
    ...
    for predImageFile in predInfo:
        ...
        for (gtNum,gtInstance) in enumerate(gtInstancesOrig[labelName]):
            intersection_area = np.logical_and( gtNp == gtInstance["instID"] , boolPredInst)
            intersection = np.count_nonzero(intersection_area)
            ...
            if (intersection > 0):
                ...
                gtCopy["intersection_area"]   = intersection_area
                predCopy["intersection_area"] = intersection_area
...
def evaluateMatches(matches, args, img_shape):
    ...
    for dI,(minRegionSize,distanceTh,distanceConf) in enumerate(zip(minRegionSizes,distThs,distConfs)):
        for (oI,overlapTh) in enumerate(overlaps):
            for (lI,labelName) in enumerate(args.instLabels):
                ...
                for img in matches:
                    correctness_img = np.ones(img_shape)
                    correctness_filename = os.path.join(os.path.dirname(img), os.path.basename(img) + '_correct.png')
                    ...
                    bestMatches = [None] * len(gtInstances)
                            ...
                            if overlap > overlapTh:
                                ...
                                if curMatch[gtI]:
                                    ...
                                    if confidence == maxScore:
                                        bestMatches[gtI] = pred["intersection_area"]
                    ...
                    for (gtI,gt) in enumerate(gtInstances):
                        correctness_img[gt["mask"]] = 0
                        if bestMatches[gtI] is not None:
                            correctness_img[bestMatches[gtI]] = 1
                    ...
                    print(f'Saving correctness image to {correctness_filename}')
                    Image.fromarray((correctness_img * 255).astype('uint8')).save(correctness_filename)
...
def evaluateImgLists(predictionList, groundTruthList, args):
    ...
    # evaluate matches
    img_shape = np.array(readGTImage(groundTruthList[0], args)).shape
    apScores = evaluateMatches(matches, args, img_shape)
    # averages
